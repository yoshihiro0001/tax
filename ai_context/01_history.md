...
■種別
機能追加（税務網羅性強化・視覚化・健全性スコア）

■内容

### 1. カテゴリ網羅性強化
- `welfare`（福利厚生費・法人用）カテゴリを新設
- `insurance` を「保険・年金（個人）→所得控除」と明示分離（計算ロジックと連動）
- 全カテゴリに `taxAttr`（expense/deduction/asset/non_deductible）と説明文を付与
- income テーブルに `taxable INTEGER DEFAULT 1` カラム追加（DBマイグレーション）
- 収入入力フォームに消費税区分セレクト（課税/非課税）を追加
- 消費税計算を課税売上のみに基づく正確計算に変更

### 2. 消費税の「壁」アラート（新機能）
- 課税売上の1,000万円到達率をプログレスバーで視覚化
- 700万（warning）/ 900万（danger）/ 超過（over）の3段階カラーアラート
- 非課税売上（輸出・土地等）は別途表示して除外理由を明記

### 3. 欠損金（赤字）繰越の可視化（新機能）
- 当年欠損金額・来年の最大節税効果・繰越可能年数（個人3年/法人10年）を表示
- 「赤字＝悪いこと」ではなく「節税の貯金」としてUIで伝える設計

### 4. 税務健全性スコア（新機能）
- 接待交際費比率・家賃按分比率・未分類比率・固定資産未登録を指標化
- A〜Dのグレード＋各リスク項目ごとの具体的アドバイスを表示
- 税務調査で突っ込まれやすいポイントを事前に数値化して警告

■対象
- server.js（welfare/taxable対応・calcHealthScore追加・consumptionTaxAlert・carryoverLoss・税シミュレーション拡張）
- public/app.js（welfareカテゴリ・taxable送信・renderConsumptionTaxAlert/renderCarryoverLoss/renderHealthScore）
- public/index.html（消費税区分セレクト・3新カードHTML・キャッシュv=20260218c）
- public/style.css（cta-card/carryover-card/health-score-card スタイル追加）
...
| 医療費控除・社会保険料控除 | ✅ |
| 減価償却（月割・売却対応） | ✅ |
| 欠損金繰越（個人3年/法人10年） | ✅ 今回追加 |
| 消費税の壁アラート（1,000万） | ✅ 今回追加 |
| 税務健全性スコア（A〜D） | ✅ 今回追加 |
| 支払カレンダー（月別タイムライン） | ✅ |
| welfare（法人福利厚生費） | ✅ 今回追加 |
| 役員報酬最適化提案 | ✅ |

■デプロイ記録
- 構文チェック（server.js / app.js）→ エラーなし
- キャッシュバスティング更新: v=20260219b → v=20260218c
- コミット: de5ff77
- push: origin/main へ反映
- bash deploy.sh → PM2 tax (id:4) online 確認
- 検証①〜⑤ 全通過（HTTP 200 / キャッシュ app.js?v=20260218c）
...
## Phase 8: 令和８年度申告書対応 — カテゴリ体系再設計・オンボーディング・レポート簡素化（2026-02-22）

### 設計思想
令和８年度の市民税・県民税申告書のすべての項目をカバーできるよう、収入・支出カテゴリを全面的に再設計。
「カテゴリ整理のプロ」として、階層型（特大→大→中→小）で整理し、網羅的だがUIはシンプルに。

### 1. DB: user_profiles テーブル追加（server.js）
- 氏名、カナ、生年月日、住所、電話、マイナンバー、職業
- 確定申告の種別（白色/青色）、青色控除額
- 配偶者有無・収入、扶養親族（一般/特定/老人）
- 障害者/学生/ひとり親/寡婦ステータス
- 自治体、40歳以上、オンボーディング完了フラグ

### 2. 収入タイプ全面拡張（server.js INCOME_TAX_ATTR）
申告書の全所得区分をカバー:
事業(営業等/農業), 給与, 不動産, 利子, 配当, 株式等譲渡, FX・先物, 公的年金,
雑(業務/その他), 総合譲渡(短期/長期), 一時, 山林, 退職, 助成金, 還付金
各項目に formField（申告書の欄名）を紐付け。

### 3. 支出カテゴリ拡張（server.js TAX_ATTRIBUTES / app.js）
- 生命保険料(life_insurance), 地震保険料(earthquake_insurance), 寄附金(donation) を追加
- 階層グループ: 事業経費 / 控除につながる支出 / 税金 / 固定資産

### 4. プロフィールAPI（server.js）
- GET /api/profile, PUT /api/profile エンドポイント追加
- GET /api/auth/me にprofile情報を含める

### 5. 税務エンジン拡張（server.js /api/tax-simulation）
- プロフィール情報から配偶者控除・扶養控除・ひとり親控除・青色申告特別控除を自動適用
- 生命保険・地震保険・寄附金の支出カテゴリから自動控除計算
- 全所得区分の分離課税/総合課税を正しく処理
- 一時所得（特別控除50万+1/2課税）、長期譲渡（1/2課税）の特殊ロジック

### 6. フロントエンド: 階層型カテゴリUI（app.js / index.html / style.css）
- expenseGroups: 事業経費 / 控除につながる支出 / 税金 / 固定資産
- incomeGroups: よく使う / 金融・投資 / 年金・一時・その他 / 特殊な所得（折りたたみ）
- 確認画面・手動入力・収入モーダル・編集モーダル全てに階層カテゴリ描画

### 7. オンボーディング（app.js / index.html / style.css）
- 初回ログイン時に名前→生年月日→確定申告種別→収入源を段階的に聞く
- いつでもスキップ可能、後から設定で変更可能
- プログレスバー付き

### 8. レポート簡素化（index.html / app.js）
- 「税務健全性スコア」カード削除（素人に不要）
- 「節税提案」「支出の節税効果」カード削除（HTML上）
- 支払カレンダー → 「直近の支払い」3件のみ表示、展開で全件
- 収支フロー、税内訳、控除、減価償却、AI相談 → 折りたたみ化

### 9. 設定に「基礎情報」プロフィールカード追加
- 氏名、カナ、生年月日、住所、電話、職業、確定申告種別、配偶者、扶養親族等を一元管理

### 10. 00_constitution.md 更新
- ゴールを「令和８年度申告書の自動反映」に明記
- カテゴリ整理方針を追加
- オンボーディング・レポート簡素化の思想を追加

■対象ファイル
- server.js（user_profiles, INCOME_TAX_ATTR拡張, TAX_ATTRIBUTES拡張, profile API, 税務エンジン）
- public/app.js（階層カテゴリ, オンボーディング, プロフィールフォーム, 折りたたみ, 支払い3件表示）
- public/index.html（レポート簡素化, 折りたたみ, プロフィールカード, オンボーディング, 階層セレクター）
